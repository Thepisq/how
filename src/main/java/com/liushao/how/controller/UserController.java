...
import com.liushao.how.service.UserService;
...
public class UserController {
    ...
    public Result login(@RequestBody User user) {
        user = userService.login(user.getMobilePhone(), user.getPassword());
        if (user == null) {
            return new Result(false, StatusCode.LOGINERROR, "登录失败");
        }
        String token = jwtUtil.createJWT(user.getId(), user.getMobilePhone(), "user");
    ...
    public Result sendSms(@PathVariable String mobile) {
        if (userService.findByMobile(mobile) != null) {
            return new Result(false, StatusCode.ERROR, "该手机号已被注册！");
        }
